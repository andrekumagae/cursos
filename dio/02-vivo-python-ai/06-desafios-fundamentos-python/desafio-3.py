...
def validate_numero_telefone(phone_number):
   pattern1 = "([0-9][0-9]) 9[0-9][0-9][0-9][0-9]-[0-9][0-9][0-9][0-9]"
   pattern = "\(\d\d\)\s9\d\d\d\d\-\d\d\d\d"
   resultado = ""
   ...
   if re.match(pattern, phone_number):
    resultado = "Número de telefone válido."
   else:
    resultado = "Número de telefone inválido."
   return resultado
